...
import { BaseRepository, runInTx } from "../../common/db/base.repository";
...
class ProbeRepo extends BaseRepository {
  async insertPatient(med: string, first: string, last: string, tx?: any) {
    ...
    const params = [this.tenantContext.tenantId(), med, first, last, ACTOR, "c"];
  ...
  countPatients(tx?: any) {
    ...
    const run = (c: any) => c.query(sql, [this.tenantContext.tenantId()]).then((r: any[]) => Number(r[0].n));
    return tx ? run(tx) : this.withIsolation(run);
  }
  ...
  rawCountLike(pattern: string) {
    return this.withIsolation((tx) =>
      tx.query<{ n: number }>(`SELECT count(*)::int AS n FROM health.patients WHERE medical_record LIKE $1`, [pattern]).then((r) => Number(r[0].n)));
  }
}

function runAs<T>(tenantCtx: TenantContext, tenantId: string, country: string, entity: string, fn: () => Promise<T>): Promise<T> {
  return new Promise<T>((res, rej) => {
    requestStorage.run({ correlationId: "c", requestId: "r", startedAt: Date.now(), method: "T", path: "/", ip: "127.0.0.1" }, () =>
      tenantCtx.run({ userId: ACTOR, email: "a@b.c", role: "doctor", permissions: ["patient:register", "patient:read"],
        tenantId, countryCode: country, entityCode: entity, globalUserId: ACTOR } as any,
        () => fn().then(res, rej)));
  });
}
...
describe("Transaction isolation (Phase 2A)", () => {
  ...
  beforeAll(async () => {
    ...
    for (const f of fs.readdirSync(MIG_DIR).filter((f) => f.endsWith(".up.sql")).sort())
    ...
    audit = new AuditService(conn, tenantCtx);
  ...
  afterAll(async () => {
    await conn.exec("RESET ROLE; DROP ROLE IF EXISTS rls_app;").catch(() => null);
  });
  ...
  it("cross-tenant isolation: tenant B cannot see tenant A's patients via direct query under B's GUCs", () =>
    runAs(tenantCtx, TENANT_B, "TZ", "H1", async () => {
      ...
      const r = await repo.withIsolation((tx) => tx.query<{ n: number }>(
