...
describe("QueueService", () => {
  it("defaults to memory backend and processes jobs", async () => {
    ...
    let processed = 0;
    q.registerHandler("test.action", async () => { processed++; });
    q.startWorkers(1);
    const id = await q.enqueue({
      idempotencyKey: "k-1", correlationId: "cid", requestId: "rid",
      globalUserId: "u", tenantId: "t", provider: "test", action: "test.action", payload: {},
    });
  ...
  it("enforces idempotency — duplicate idempotencyKey returns same id and runs once", async () => {
    const q = new QueueService({ get: () => undefined } as any);
    let count = 0;
    q.registerHandler("idem.action", async () => { count++; });
    q.startWorkers(1);
    const a = await q.enqueue({
      idempotencyKey: "idem-1", correlationId: "c", requestId: "r",
      globalUserId: "u", tenantId: "t", provider: "test", action: "idem.action", payload: {},
    });
  ...
  it("poison jobs route to dead-letter after maxAttempts", async () => {
    const q = new QueueService({ get: () => undefined } as any);
    q.registerHandler("poison.action", async () => { throw new Error("POISON"); });
    q.startWorkers(1);
    await q.enqueue({
      idempotencyKey: "p-1", correlationId: "c", requestId: "r",
      globalUserId: "u", tenantId: "t", provider: "test", action: "poison.action",
      payload: {}, maxAttempts: 2, backoffMs: 5,
    });
  ...
  it("production without Redis BLOCKS enqueue", () => {
    const q = new QueueService({ get: (k: string) => k === "NODE_ENV" ? "production" : undefined } as any);
    expect(q.backend).toBe("blocked");
    expect(q.enqueue({
      idempotencyKey: "x", correlationId: "c", requestId: "r",
      globalUserId: "u", tenantId: "t", provider: "test", action: "x", payload: {},
...
async function waitUntil(cond: () => boolean, timeout: number): Promise<void> {
  ...
  while (!cond() && Date.now() - start < timeout) await new Promise((r) => setTimeout(r, 10));
