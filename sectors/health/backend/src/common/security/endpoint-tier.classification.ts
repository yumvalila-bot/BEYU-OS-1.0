...
export function classifyEndpoint(
  ...
  _hasGovernance: boolean = false,
  _hasHcm: boolean = false,
  _hasMfa: boolean = false,
  _hasClinicalSafety: boolean = false,
  ...
  if (isPublic) {
    if (p.includes("/health") || p.includes("live") || p.includes("ready")) {
      return pub("PUBLIC", verb, path, controller, {
        rateLimit: null,
        audit: false,
      });
    }
  ...
  if (
    /integration|webhook|fhir\/r4|fhir\/r5/.test(p) &&
  ...
  if (
    /tenant|rbac|role|permission|config|admin|audit\/export|ai\/configure|breakglass|trustee|board/.test(
      p,
    )
  ) {
    return tier(
  ...
  if (
    /\/(encounter|clinical|pharmacy\/dispense|patients|records|consent|incident|observ|problem|allergy|vital|diagnos|procedure|note|sign|ambulance\/requests|dialysis\/sessions|lab\/results|imaging\/reports|eye-exams\/:id\/sign)/i.test(
      p,
    ) ||
    // Pharmacy dispense already matches; include dispense/rx prescribe endpoints
    (/\/rx|dispens|prescription|medication[^/]*$/.test(p) &&
      verb !== "GET" &&
      !/stock|items/.test(p))
  ) {
    return tier(
  ...
  if (
    /compliance|reporting|mtuha|public[-_]?health|notifiable|surveill/.test(p)
  ) {
    return tier(
      "PRIVILEGED",
      verb,
...
function base(overrides?: Partial<RequiredControls>): RequiredControls {
  return {
    ...
    globalUserId: true,
    practitioner: false,
    professionalLicence: false,
    facility: false,
    scopeOfPractice: false,
    governanceAuthorization: false,
    hcmAuthorization: false,
    mfaStepUp: false,
    csrf: true,
    clinicalSafetyGate: false,
    consent: false,
    legalHold: false,
    audit: true,
    idempotency: false,
...
function pub(
  ...
  return {
    ...
    required: base({
      ...
      entityIsolation: false,
      countryIsolation: false,
      globalUserId: false,
      csrf: false,
      audit: false,
      ...ov,
...
function inferAdapter(p: string): string | null {
  ...
  if (/payment|mobile[-_]?money|mpesa|tigopesa|airtel/.test(p))
    return "payment";
