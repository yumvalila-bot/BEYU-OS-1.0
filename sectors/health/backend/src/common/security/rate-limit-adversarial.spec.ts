...
describe("Rate limiter — deterministic + fail-closed", () => {
  ...
  it("in-memory limiter enforces limit and 429s past it", async () => {
    await bed.run(async () => {
      ...
      await expect(rl.hit(cfg)).rejects.toHaveProperty(
        "response.code",
        "RATE_LIMITED",
      );
    });
  ...
  it("keys are isolated across IPs, users, and tenants", async () => {
    await bed.run(async () => {
      ...
      rl.resetAll();
      for (let i = 0; i < 10; i++)
        await rl.hit(buildPolicy("POST", "/api/w", "ip", "ip1"));
  ...
  it("exponential lockout applies after lockoutThreshold", () => {
    const p = buildPolicy("POST", "/api/auth/login", "email", "a@b.c", {
      lockoutHits: 5,
    });
  ...
  it("RATE_LIMIT_BACKEND=redis without REDIS_URL fails closed at construction", () => {
    const cfgStub = {
      get: (k: string, d?: any) => (k === "RATE_LIMIT_BACKEND" ? "redis" : d),
    };
    expect(() => new RateLimiter(bed.conn, cfgStub as any)).toThrow(
      /REDIS_URL/,
    );
  });
