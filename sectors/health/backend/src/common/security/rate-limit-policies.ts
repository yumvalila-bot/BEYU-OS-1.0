...
const POLICIES: Record<string, RateLimitPolicy> = {
  // ── Auth / credentials ──────────────────────────────────────────────────
  "POST:/auth/login":    { windowMs: 60_000, limit: 10, lockoutThreshold: 5, lockoutWindowMs: 15 * 60_000, reason: "auth_login_burst" },
  "POST:/auth/register": { windowMs: 60_000, limit: 5,  reason: "patient_registration_burst" },
  "POST:/auth/refresh":  { windowMs: 60_000, limit: 30, reason: "auth_refresh_burst" },
  "POST:/auth/logout":   { windowMs: 60_000, limit: 30, reason: "auth_logout_burst" },
  "GET:/auth/csrf-token":{ windowMs: 60_000, limit: 120, reason: "csrf_token_burst" },

  // ── MFA ─────────────────────────────────────────────────────────────────
  "POST:/auth/mfa/challenge": { windowMs: 60_000, limit: 6, lockoutThreshold: 5, lockoutWindowMs: 30 * 60_000, reason: "mfa_challenge_burst" },
  "POST:/auth/mfa/verify":    { windowMs: 60_000, limit: 6, lockoutThreshold: 5, lockoutWindowMs: 30 * 60_000, reason: "mfa_verify_burst" },
  "POST:/auth/mfa/enroll":    { windowMs: 60_000, limit: 10, reason: "mfa_enroll_burst" },
  "POST:/auth/mfa/recovery":  { windowMs: 60_000, limit: 5,  lockoutThreshold: 3, lockoutWindowMs: 60 * 60_000, reason: "mfa_recovery_burst" },

  // ── Password / recovery (reserved for endpoint) ─────────────────────────
  "POST:/auth/password-reset":        { windowMs: 60_000, limit: 5, lockoutThreshold: 3, lockoutWindowMs: 30 * 60_000, reason: "password_reset_burst" },
  "POST:/auth/password-reset/confirm":{ windowMs: 60_000, limit: 5, lockoutThreshold: 3, lockoutWindowMs: 30 * 60_000, reason: "password_reset_confirm_burst" },

  // ── Clinical writes ─────────────────────────────────────────────────────
  "clinical_write":   { windowMs: 60_000, limit: 120, reason: "clinical_write_burst" },
  "appointment_book": { windowMs: 60_000, limit: 30,  reason: "appointment_booking_burst" },
  "prescription_write":{ windowMs: 60_000, limit: 60, lockoutThreshold: 20, lockoutWindowMs: 15*60_000, reason: "prescription_write_burst" },
  "lab_operation":    { windowMs: 60_000, limit: 120, reason: "lab_operation_burst" },

  // ── Financial / billing ─────────────────────────────────────────────────
  "billing_write":    { windowMs: 60_000, limit: 30, lockoutThreshold: 10, lockoutWindowMs: 30*60_000, reason: "billing_write_burst" },

  // ── External integrations / outbound ────────────────────────────────────
  "external_submission":{ windowMs: 60_000, limit: 20, lockoutThreshold: 10, lockoutWindowMs: 15*60_000, reason: "external_submission_burst" },
  "public_health_submission":{ windowMs: 60_000, limit: 10, reason: "public_health_submission_burst" },

  // ── AI / governed services ──────────────────────────────────────────────
  "ai_invocation":    { windowMs: 60_000, limit: 20, lockoutThreshold: 10, lockoutWindowMs: 15*60_000, reason: "ai_invocation_burst" },

  // ── Admin / governance ──────────────────────────────────────────────────
  "admin_sensitive":  { windowMs: 60_000, limit: 30, lockoutThreshold: 10, lockoutWindowMs: 15*60_000, reason: "admin_sensitive_burst" },
...
export function policyFor(method: string, path: string): RateLimitPolicy {
  ...
  if (/(lab|laboratory)/.test(p) && method !== "GET") return POLICIES["lab_operation"];
  ...
  if (/mtuha|public[-_]?health|notifiable|surveill|fhir/.test(p) && method !== "GET")
    return POLICIES["public_health_submission"];
  if (/integration|webhook/.test(p) && method !== "GET")
    return POLICIES["external_submission"];
  if (/admin|tenant|rbac|role|trustee|breakglass|board/.test(p) && method !== "GET")
    return POLICIES["admin_sensitive"];
  if (/patient|encounter|clinical|observ|problem|allergy|medication|radiolog|imaging|dialys|ophthal|optical|telehealth|ambulance|consent|incident|records|note/.test(p) && method !== "GET")
