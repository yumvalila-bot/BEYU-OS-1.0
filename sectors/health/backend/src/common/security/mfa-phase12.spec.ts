...
type State = "ENGINEERING_READY" | "PARTIALLY_IMPLEMENTED" | "MISSING" | "EXTERNAL_BLOCKED";
...
describe("MFA + session security matrix (Phase 12 Wave 4)", () => {
  ...
  beforeAll(() => {
    ...
    const svSpec = read("modules/identity/security-version.adversarial.spec.ts");
    const mfaCtl = read("modules/auth/mfa.controller.ts");

    matrix = [
      {
        point: "security_version",
        status: /security_version/.test(svSpec) && /bumpSecurityVersion/.test(svSpec) ? "ENGINEERING_READY" : "MISSING",
        evidence: "security-version.adversarial.spec.ts (bump + invalidate)",
      },
      {
        point: "stale_jwt_session_invalidation",
        status: /AUTHORIZATION_CHANGED|stale refresh/.test(svSpec) ? "ENGINEERING_READY" : "MISSING",
        evidence: "security-version.adversarial.spec.ts (rotateSession/assertSessionActive reject stale sv)",
      },
      {
        point: "stepup_user_binding",
        status: /c\.user_id\s*=\s*u\.global_user_id/.test(guard) ? "ENGINEERING_READY" : "MISSING",
        evidence: "MfaStepUpGuard joins mfa_challenges.user_id = users.global_user_id",
      },
      {
        point: "stepup_session_binding",
        status: /session_id/.test(guard) ? "ENGINEERING_READY" : "MISSING",
        evidence: "MfaStepUpGuard compares session_id (MFA_SESSION_CROSSOVER on mismatch)",
      },
      {
        point: "stepup_tenant_binding",
        status: /c\.tenant_id\s*=\s*\$4|c\.tenant_id::text\s*=\s*\$4/.test(guard) ? "ENGINEERING_READY" : "PARTIALLY_IMPLEMENTED",
        evidence: "MfaStepUpGuard binds mfa_challenges.tenant_id to req.user.tenantId (Wave 4)",
      },
      {
        point: "stepup_purpose_binding",
        status: /c\.purpose\s*=\s*\$2/.test(guard) ? "ENGINEERING_READY" : "MISSING",
        evidence: "MfaStepUpGuard filters mfa_challenges.purpose = action",
      },
      {
        point: "replay_protection",
        status: /consumed_at IS NULL/.test(guard) && /reuse of same challenge|already consumed/i.test(mfaSpec) ? "ENGINEERING_READY" : "MISSING",
        evidence: "consumed challenge rejected (mfa.adversarial.spec.ts replay test)",
      },
      {
        point: "expiry",
        status: /expires_at > now\(\)/.test(guard) && /MFA_CHALLENGE_EXPIRED/.test(mfaSpec) ? "ENGINEERING_READY" : "MISSING",
      ...
      {
        point: "lockout",
        status: /assertLockout|recordFailureTx|locked_until/.test(svc) ? "ENGINEERING_READY" : "MISSING",
        evidence: "exponential backoff lockout (BASE_LOCKOUT_MS scaling)",
      },
      {
        point: "recovery_code_single_use",
        status: /used_at/.test(svc) && /redeems once; reuse rejected/.test(mfaSpec) ? "ENGINEERING_READY" : "MISSING",
        evidence: "recovery code redeem once; reuse rejected (mfa.adversarial.spec.ts)",
      },
      {
        point: "admin_reset_authorization",
        status: /@RequirePermission\("tenant:admin"\)/.test(mfaCtl) && /@RequiresMfaStepUp\("mfa:admin:reset"\)/.test(mfaCtl) ? "ENGINEERING_READY" : "MISSING",
        evidence: "mfa.controller admin/reset: tenant:admin + MFA step-up (Wave 1)",
      },
      {
        point: "admin_reset_audit",
        status: /mfa\.admin\.reset/.test(svc) && /resetBy/.test(svc) ? "ENGINEERING_READY" : "MISSING",
        evidence: "adminReset writes audit event with reason + resetBy actor",
      },
      {
        point: "high_risk_endpoint_coverage",
        status: "PARTIALLY_IMPLEMENTED",
        evidence: "FINANCIAL (billing) + mfa admin reset carry @RequiresMfaStepUp; full ADMINISTRATIVE/AI_HIGH_RISK sweep pending",
      },
    ];
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        methodology: "MFA + session security controls verified against source + existing adversarial test coverage; evidence-derived states",
        summary: matrix.reduce((acc, m) => { acc[m.status] = (acc[m.status] ?? 0) + 1; return acc; }, {} as Record<string, number>),
        points: matrix,
      }, null, 2),
    );
  });
  ...
  it("core MFA controls (security_version, replay, expiry, recovery, admin reset) are ENGINEERING_READY", () => {
    const core = ["security_version", "replay_protection", "expiry", "recovery_code_single_use", "admin_reset_authorization", "admin_reset_audit"];
    for (const c of core) {
