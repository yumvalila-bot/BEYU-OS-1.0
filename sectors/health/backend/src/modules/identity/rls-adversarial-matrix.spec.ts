...
describe("RLS adversarial matrix — per-table 10-case isolation probe", () => {
  ...
  beforeAll(async () => {
    ...
    await bed.run(async () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({ generated: new Date().toISOString(), tables: results }, null, 2),
      );
      // eslint-disable-next-line no-console
      console.log("RLS adversarial probed", results.length, "tables; names:", results.map(r=>r.table).slice(0,5).join(","), "...", results.map(r=>r.table).slice(-5).join(","));
    });
  }, 60000);
  ...
  it("every health.* table reports 10-case status with no FAIL on core isolation cases for critical tables", async () => {
    ...
    const critical = ["patients", "encounters", "audit_log", "dispenses",
                      "lab_orders", "lab_tests", "imaging_orders", "invoices"];
...
async function probeTable(bed: any, table: string): Promise<any> {
  ...
  try {
    ...
    await bed.conn.exec("RESET app.tenant_id; RESET app.entity_code; RESET app.country_code;");
    ...
    await bed.conn.exec(`SET app.tenant_id='${bed.conn.__testActor?.tenantId ?? "11111111-1111-1111-1111-111111111111"}';`);
    ...
    rec.cases.insert_wrong_tenant_blocked = await tryInsertWrongTenant(bed, table, colNames);
    ...
    rec.cases.update_wrong_tenant_blocked = rec.cases.wrong_tenant_zero === true;
...
async function safeCount(bed: any, table: string): Promise<number | "ERROR"> {
  try {
    const rows = await bed.conn.query(`SELECT count(*)::int AS n FROM health.${identifier(table)}`);
...
async function tryInsertWrongTenant(bed: any, table: string, cols: string[]): Promise<boolean | string> {
  ...
  try {
    ...
    // Best-effort cleanup.
    try {
      await bed.conn.query(`DELETE FROM health.${identifier(table)} WHERE id=$1`, [newId]);
    } catch { /* ignore */ }
    return false;
  } catch (e: any) {
    const msg = String(e?.message ?? "");
    if (/permission denied|row security|violates row-level security|check option/i.test(msg)) {
      return true;
    }
