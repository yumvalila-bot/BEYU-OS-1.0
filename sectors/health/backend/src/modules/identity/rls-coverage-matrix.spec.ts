...
describe("RLS coverage matrix — every health.* table has RLS + fail-closed isolation", () => {
  ...
  it("every health.* table in migrations has RLS enabled and at least one policy", async () => {
    await bed.run(async () => {
      ...
      for (const f of files) {
        ...
        const re = /CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?health\.([a-z0-9_]+)/gi;
      ...
      for (const t of tableSet) {
        ...
        if (!r) { uncovered.push(t); continue; }
        if (!r.rls_enabled) uncovered.push(t);
        if (Number(r.policy_count) < 1) missingPolicy.push(t);
      }
      if (uncovered.length) throw new Error("health.* tables missing RLS: " + uncovered.join(","));
      if (missingPolicy.length) throw new Error("health.* tables without a policy: " + missingPolicy.join(","));
    });
  });
  ...
  it("without app.tenant_id GUC, selecting representative tenant-scoped tables returns 0 rows (fail-closed default)", async () => {
    ...
    for (const t of probes) {
      try {
        const rows = await bed.conn.query(`SELECT count(*)::int AS n FROM health.${t}`);
  ...
  it("emits machine-readable RLS coverage report", async () => {
    await bed.run(async () => {
      ...
      const report = rows.map((r: any) => ({ table: r.table_name, rls: r.rls_enabled, policies: Number(r.policy_count) }));
      ...
      try {
        ...
        fs.writeFileSync(
          path.resolve(__dirname, "..", "..", "..", "coverage", "rls-matrix.json"),
          JSON.stringify({ generated: new Date().toISOString(), tables: report }, null, 2),
        );
      } catch {}
    });
  });
