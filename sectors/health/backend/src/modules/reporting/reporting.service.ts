...
export class ReportingService {
  constructor(private readonly repo: ReportingRepository, private readonly audit: AuditService) {}

  async generatePeriodReport(periodStart: string, periodEnd: string): Promise<PeriodReport> {
    if (!periodStart || !periodEnd) throw DomainError.validation("period_start / period_end required");
    if (new Date(periodEnd) <= new Date(periodStart)) throw DomainError.validation("period_end must be after period_start");
    return this.repo.withIsolation(async (tx) => {
      ...
      const pharmacy = await this.repo.pharmacyDispenses(periodStart, periodEnd, tx);
      const ambulance = await this.repo.ambulanceVolume(periodStart, periodEnd, tx);
      ...
      const report: PeriodReport = {
        ...
        opd: opd ?? {}, lab, imaging, pharmacy, ambulance,
        mapping_status: "incomplete", missing_mappings: missing,
      };
      await this.audit.record(tx, { operation: "reporting.mtuha.generate", resourceType: "mtuha_report", resourceId: `${periodStart}:${periodEnd}`, after: { period_start: periodStart, period_end: periodEnd } });
      return report;
    });
  }

  async markSubmitted(input: { period_start: string; period_end: string; book_code: string; file_ref?: string }) {
    return this.repo.withIsolation(async (tx) => {
      const rec = await this.repo.recordReport({ ...input, status: "submitted" }, tx);
      await this.audit.record(tx, { operation: "reporting.mtuha.submitted", resourceType: "mtuha_report", resourceId: rec.report_id, after: rec });
      return rec;
    });
  }
