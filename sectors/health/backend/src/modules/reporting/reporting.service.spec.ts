...
describe("ReportingService (MTUHA)", () => {
  ...
  it("rejects invalid period (end <= start)", () =>
    bed.run(async () => {
      await expect(svc.generatePeriodReport("2026-02-01", "2026-01-01")).rejects.toBeInstanceOf(DomainError);
    }));
  ...
  it("produces structured aggregates and fails closed on mapping (mapping_status=incomplete)", () =>
    bed.run(async () => {
      ...
      await bed.conn.query(
        ...
        [bed.tenantCtx.tenantId(), `E${Date.now()}`, p.patient_id, "00000000-0000-0000-0000-000000000001"],
      );
  ...
  it("markSubmitted creates an audit record of submission; does NOT POST to an external endpoint", () =>
    bed.run(async () => {
      const rec = await svc.markSubmitted({ period_start: "2026-01-01", period_end: "2026-01-31", book_code: "MTUHA-30", file_ref: "mtuha30_jan.csv" }) as any;
