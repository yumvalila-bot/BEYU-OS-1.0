...
export class OutboxDispatcherService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit(): void {
    ...
    if (!this.endpoint()) {
      this.logger.log(
        "governed event dispatch disabled — BEYU_EVENTS_ENDPOINT not configured (outbox accumulates; reconciliation surfaces backlog)",
      );
      return;
    }
    if (interval <= 0) {
      this.logger.log(
        "governed event dispatch interval disabled (BEYU_EVENTS_DISPATCH_INTERVAL_MS<=0) — dispatchDueBatch() must be invoked explicitly",
      );
      return;
    }
  ...
  private async deliver(
    ...
    "delivered" | "duplicates" | "retried" | "deadLettered" | "errors"
  > {
    ...
    try {
      ...
      const dead = err.permanent === true || attempt >= this.maxAttempts();
      if (dead) {
  ...
  private async postEvent(row: OutboxRow): Promise<{
    accepted: boolean;
    duplicate: boolean;
    eventId: string | null;
  }> {
    ...
    if (!endpoint || !secret) {
      throw Object.assign(
        new Error("BEYU_EVENTS_ENDPOINT/BEYU_EVENTS_TOKEN not configured"),
        { permanent: false },
      );
    }
    ...
    try {
      const res = await fetch(
        `${endpoint.replace(/\/$/, "")}/api/v1/internal/events`,
        {
          method: "POST",
          headers: {
      ...
      if ((e as Error).name === "AbortError") {
        throw Object.assign(
          new Error(`BEYU timeout after ${this.timeoutMs()}ms`),
          { permanent: false },
        );
      }
      ...
      attemptLog: [
        {
          ...
          at: new Date().toISOString(),
          phase: result.duplicate ? "delivered-duplicate" : "delivered",
  ...
  private async markDeadLetter(
    ...
    this.logger.error(
      `outbox row ${row.idempotency_key} DEAD-LETTERED after ${attempt} attempts: ${error}`,
    );
  }
