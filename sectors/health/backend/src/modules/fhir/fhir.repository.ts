...
export class FhirRepository extends BaseRepository {
  ...
  listPatientConditions(pid: string) {
    return this.withIsolation((tx) =>
      tx.query(
        `SELECT * FROM health.problems WHERE patient_id=$1 AND ${this.notVoided("health.problems")}`,
        [pid],
      ),
    );
  }
  listPatientObservations(pid: string) {
    return this.withIsolation((tx) =>
      tx.query(
        `SELECT * FROM health.observations WHERE patient_id=$1 AND ${this.notVoided("health.observations")} ORDER BY observed_at DESC`,
        [pid],
      ),
    );
  }
  listPatientMedicationRequests(pid: string) {
    return this.withIsolation((tx) =>
      tx.query(
        `SELECT * FROM health.medications WHERE patient_id=$1 AND ${this.notVoided("health.medications")}`,
        [pid],
      ),
    );
  }
  listPatientAllergies(pid: string) {
    return this.withIsolation((tx) =>
      tx.query(
        `SELECT * FROM health.allergies WHERE patient_id=$1 AND ${this.notVoided("health.allergies")}`,
        [pid],
      ),
    );
  }
