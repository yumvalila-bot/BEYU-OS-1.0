...
export class FhirService {
  ...
  async patient(id: string) {
    ...
    return {
      ...
      name: [{ use: "official", family: p.family_name, given: [p.given_name, p.middle_name].filter(Boolean) }],
      ...
      meta: { lastUpdated: p.updated_at, versionId: p.updated_at ? String(p.updated_at) : undefined },
    };
  }
  ...
  async encounter(id: string) {
    ...
    return {
      resourceType: "Encounter", id: e.encounter_id,
      status: e.status === "in_progress" ? "in-progress" : e.status,
      class: { system: "http://terminology.hl7.org/CodeSystem/v3-ActCode", code: e.kind === "emergency" ? "EMER" : e.kind === "inpatient" ? "IMP" : "AMB" },
      subject: { reference: `Patient/${e.patient_id}` },
      appointment: e.appointment_id ? [{ reference: `Appointment/${e.appointment_id}` }] : undefined,
  ...
  async conditions(pid: string) {
    ...
    return rows.map((r: any) => ({
      resourceType: "Condition", id: r.problem_id,
      ...
      code: { coding: r.code ? [{ system: `http://${r.code_system}.org`, code: r.code, display: r.description }] : undefined, text: r.description },
  ...
  async observations(pid: string) {
    ...
    return rows.map((r: any) => ({
      resourceType: "Observation", id: r.observation_id,
      status: r.signed_at ? "final" : "preliminary",
      category: [{ coding: [{ system: "http://terminology.hl7.org/CodeSystem/observation-category", code: r.category }] }],
      code: { coding: [{ system: r.code_system === "LOINC" ? "http://loinc.org" : "http://beyu.health/fhir/CodeSystem/local", code: r.code, display: r.display }] },
      subject: { reference: `Patient/${r.patient_id}` },
      effectiveDateTime: r.observed_at,
      valueQuantity: r.value_numeric != null ? { value: Number(r.value_numeric), unit: r.value_units ?? undefined } : undefined,
      valueString: r.value_text ?? undefined,
      interpretation: r.abnormal_flag ? [{ coding: [{ system: "http://terminology.hl7.org/CodeSystem/v3-ObservationInterpretation", code: r.abnormal_flag === "high" || r.abnormal_flag === "critical_high" ? "H" : r.abnormal_flag === "low" || r.abnormal_flag === "critical_low" ? "L" : "N" }] }] : undefined,
    }));
  }
  ...
  async medicationRequests(pid: string) {
    ...
    return rows.map((r: any) => ({
      resourceType: "MedicationRequest", id: r.medication_id,
      status: r.status === "active" ? "active" : r.status,
      intent: "order",
      medicationCodeableConcept: { text: r.name },
      subject: { reference: `Patient/${r.patient_id}` },
      dosageInstruction: [{ text: `${r.dose} ${r.route ?? ""} ${r.frequency ?? ""}`.trim(), timing: r.frequency ? { code: { text: r.frequency } } : undefined }],
  ...
  async allergyIntolerances(pid: string) {
    ...
    return rows.map((r: any) => ({
      resourceType: "AllergyIntolerance", id: r.allergy_id,
      clinicalStatus: { coding: [{ system: "http://terminology.hl7.org/CodeSystem/allergyintolerance-clinical", code: r.status === "active" ? "active" : "resolved" }] },
      category: [r.category],
      criticality: r.severity === "life_threatening" ? "high" : r.severity === "severe" ? "high" : r.severity === "moderate" ? "low" : "low",
      code: { text: r.substance_name },
      patient: { reference: `Patient/${r.patient_id}` },
      reaction: r.reaction ? [{ description: r.reaction, severity: r.severity === "life_threatening" ? "severe" : r.severity }] : undefined,
    }));
  }

  async bundle(patientId: string) {
    const entries: any[] = [];
    const p = await this.patient(patientId); entries.push({ resource: p });
    for (const c of await this.conditions(patientId)) entries.push({ resource: c });
    for (const o of await this.observations(patientId)) entries.push({ resource: o });
    for (const m of await this.medicationRequests(patientId)) entries.push({ resource: m });
    for (const a of await this.allergyIntolerances(patientId)) entries.push({ resource: a });
