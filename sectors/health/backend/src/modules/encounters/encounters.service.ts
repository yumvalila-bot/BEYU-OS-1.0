...
import { EncounterRepository, Encounter, CreateEncounterInput } from "./encounter.repository";
...
export class EncountersService {
  ...
  async start(input: CreateEncounterInput): Promise<Encounter> {
    ...
    if (input.appointment_id) {
      const active = await this.repo.findActiveByAppointment(input.appointment_id);
      if (active) {
  ...
  async complete(id: string, disposition: string, presentIllness?: string): Promise<Encounter> {
    ...
    const allowed = ["discharged", "admitted", "referred", "died", "absconded", "ama"];
    if (!allowed.includes(disposition)) {
      throw DomainError.validation(`disposition must be one of: ${allowed.join(",")}`);
    }
  ...
  async cancel(id: string): Promise<Encounter> {
    ...
    if (e.status !== "in_progress" && e.status !== "checked_in") {
      throw DomainError.invalidState(`Cannot cancel encounter in status ${e.status}`);
    }
