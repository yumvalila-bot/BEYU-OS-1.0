...
export class ClinicalService {
  ...
  listProblems(patientId: string) { return this.repo.listProblems(patientId); }
  async addProblem(input: Record<string, unknown>) {
    ...
    this.audit.record(/* tx */ this.repo as unknown as Parameters<AuditService["record"]>[0], {
      operation: "problem.add", resourceType: "problem", resourceId: p.problem_id, after: p,
    }).catch((e) => {
      ...
      console.warn("audit write failed (follow-up to make atomic):", e.message);
    });
    return p;
  }

  listObservations(patientId: string, category?: string) { return this.repo.listObservations(patientId, category); }
  async addObservation(input: Record<string, unknown>) {
    return this.repo.addObservation(input);
  }

  listMedications(patientId: string, activeOnly = true) { return this.repo.listMedications(patientId, activeOnly); }
  ...
  listAllergies(patientId: string) { return this.repo.listAllergies(patientId); }
