    ...
    return this.withIsolation((tx) =>
      tx.query<Problem>(
        ...
        [
          ...
          input.patient_id, input.encounter_id ?? null,
          input.code_system ?? "ICD-10", input.code ?? null, input.description,
          input.onset_date ?? null, input.severity ?? null, input.note ?? null,
          actor, cid,
        ],
      ),
    ).then((r) => r[0]);
    ...
    return this.withIsolation((tx) =>
      tx.query<Observation>(
        ...
        [
          this.tenantContext.tenantId(),
          input.patient_id, input.encounter_id ?? null,
          input.code_system ?? "LOINC", input.code, input.display ?? null,
          input.value_numeric ?? null, input.value_text ?? null, input.value_units ?? null,
          input.abnormal_flag ?? null,
          input.category ?? "vital-signs", input.note ?? null,
          actor, cid,
        ],
      ),
    ).then((r) => r[0]);
    ...
    return this.withIsolation((tx) =>
      tx.query<Medication>(
        ...
        [
          this.tenantContext.tenantId(),
          input.patient_id, input.encounter_id ?? null,
          input.code_system ?? "RXNORM", input.code ?? null, input.name,
          input.dose, input.route ?? null, input.frequency ?? null,
          input.duration ?? null, input.quantity ?? null,
          input.refills ?? 0, input.prn ?? false,
    ...
    return this.withIsolation((tx) =>
      tx.query<Allergy>(
        ...
        [
          this.tenantContext.tenantId(),
          input.patient_id, input.encounter_id ?? null,
          input.substance_code_system ?? "RXNORM", input.substance_code ?? null,
          input.substance_name,
          input.category ?? "medication", input.severity ?? "mild",
          input.reaction ?? null, input.note ?? null,
          actor, cid,
        ],
      ),
    ).then((r) => r[0]);
