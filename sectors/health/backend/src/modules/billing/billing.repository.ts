...
export class BillingRepository extends BaseRepository {
  listServices() {
    return this.withIsolation((tx) => tx.query(`SELECT * FROM health.billable_services WHERE ${this.notVoided("health.billable_services")} ORDER BY category, name`));
  }
  createService(input: Record<string, unknown>, tx: Tx) {
    ...
    return tx.query(
      ...
      [this.tenantContext.tenantId(), input.code, input.code_system ?? "LOCAL", input.name, input.description ?? null,
        input.unit_price ?? 0, input.currency ?? "TZS", input.category ?? "other", actor, cid],
    ).then((r: any[]) => r[0]);
  }
  createInvoice(input: { patient_id: string; encounter_id?: string; items: { service_id?: string; description: string; qty?: number; unit_price: number; reference_type?: string; reference_id?: string }[]; idempotency_key?: string }, tx: Tx) {
    const actor = this.actorId();
    const cid = this.correlationId();
    const no = "INV-" + Date.now().toString(36).toUpperCase();
    let subtotal = 0;
    for (const it of input.items) subtotal += (it.qty ?? 1) * Number(it.unit_price);
    return tx.query(
      ...
      [this.tenantContext.tenantId(), no, input.patient_id, input.encounter_id ?? null, subtotal,
      ...
      for (const it of input.items) {
        ...
        await tx.query(
          ...
          [this.tenantContext.tenantId(), inv.invoice_id, it.service_id ?? null, it.description, qty, it.unit_price,
            line, it.reference_type ?? null, it.reference_id ?? null, actor, cid],
        );
      }
      return inv;
    });
  ...
  findInvoice(id: string, tx?: Tx) {
    const q = (c: Tx) => c.query(`SELECT * FROM health.invoices WHERE invoice_id=$1 AND ${this.notVoided("health.invoices")}`, [id]).then((r: any[]) => r[0] ?? null);
    return tx ? q(tx) : this.withIsolation(q);
  }
  listInvoiceItems(id: string, tx: Tx) {
    return tx.query(`SELECT * FROM health.invoice_items WHERE invoice_id=$1`, [id]);
  }
  listInvoicesForPatient(pid: string) {
    return this.withIsolation((tx) => tx.query(`SELECT * FROM health.invoices WHERE patient_id=$1 AND ${this.notVoided("health.invoices")} ORDER BY created_at DESC`, [pid]));
  }
  recordPayment(input: { patient_id: string; method: string; amount: number; reference_no?: string; idempotency_key?: string }, tx: Tx) {
    const actor = this.actorId();
    const cid = this.correlationId();
    ...
    return tx.query(
      ...
      [this.tenantContext.tenantId(), no, input.patient_id, input.method, input.amount, input.reference_no ?? null,
        input.idempotency_key ?? null, actor, cid],
    ).then((r: any[]) => r[0]);
  }
  ...
  findOutstandingInvoices(pid: string, tx: Tx) {
    return tx.query(`SELECT * FROM health.invoices WHERE patient_id=$1 AND balance>0 AND status IN ('issued','partially_paid') AND ${this.notVoided("health.invoices")} ORDER BY created_at ASC`, [pid]);
  }
  allocate(payment_id: string, invoice_id: string, amount: number, tx: Tx) {
    ...
    return tx.query(
      `INSERT INTO health.payment_allocations (tenant_id, payment_id, invoice_id, amount, created_by, correlation_id)
       VALUES ($1,$2,$3,$4,$5,$6)`,
      [this.tenantContext.tenantId(), payment_id, invoice_id, amount, actor, cid],
    );
  }
  ...
  stageFinanceEvent(event_type: string, payload: unknown, idempotency_key: string, tx: Tx) {
    return tx.query(
      ...
      [this.tenantContext.tenantId(), event_type, JSON.stringify(payload), idempotency_key],
    );
  }
}
