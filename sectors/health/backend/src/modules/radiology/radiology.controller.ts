...
export class RadiologyController {
  ...
  @Get()
  @RequirePermission("order:imaging")
  list(@Query("patient_id") p: string) {
    return this.svc.listForPatient(p);
  }
  @Post("orders")
  @RequirePermission("order:imaging")
  @RequiresClinicalSafety("radiology")
  create(@Body() d: any) {
  ...
  @RequireHcmPractitioner("radiology.report.add", {
    scope: ["radiology:report"],
  })
  @RequiresClinicalSafety("general")
  report(@Body() d: any) {
    return this.svc.addReport(d);
  }
  @Post("reports/:id/verify")
  @RequirePermission("note:sign")
  @RequiresClinicalSafety("radiology")
  @RequireHcmPractitioner("radiology.report.verify", {
    scope: ["radiology:verify", "note:sign"],
  })
  verify(
    @Param("id") id: string,
    @Body()
    d: {
      ...
      criticalEscalationLogged?: boolean;
      facilityId?: string;
