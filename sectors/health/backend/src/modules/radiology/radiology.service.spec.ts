...
describe("RadiologyService", () => {
  ...
  it("state machine ordered -> scheduled -> in_progress -> preliminary -> final (with note:sign verification)", () =>
    bed.run(async () => {
      ...
      const r = (await svc.addReport({
        imaging_order_id: o.imaging_order_id,
        findings: "Normal",
      ...
      await expect(svc.verifyReport(r.report_id)).rejects.toBeInstanceOf(
        DomainError,
      );
    }));
  ...
  it("rejects invalid state transitions", () =>
    bed.run(async () => {
      ...
      await expect(
        svc.transition(o.imaging_order_id, "in_progress"),
