...
export class RadiologyService {
  ...
  listForPatient(pid: string) {
    return this.repo.listForPatient(pid);
  }
  async createOrder(input: Record<string, unknown>) {
    if (!input.patient_id || !input.modality || !input.body_part)
      throw DomainError.validation("patient_id, modality, body_part required");
    if (input.idempotency_key) {
      const e = await this.repo.findByIdempotency(
    ...
    return this.repo.withIsolation(async (tx) => {
      ...
      await this.audit.record(tx, {
        operation: "imaging.order.create",
        resourceType: "imaging_order",
        resourceId: o.imaging_order_id,
        after: o,
      });
      return o;
    });
    ...
    return this.repo.withIsolation(async (tx) => {
      ...
      if (!TRANSITIONS[cur.status]?.has(to))
        throw DomainError.invalidState(
          `Cannot transition imaging from ${cur.status} to ${to}`,
        );
      const o = await this.repo.transitionOrder(id, to, tx);
      await this.audit.record(tx, {
        operation: "imaging.order.transition",
        resourceType: "imaging_order",
        resourceId: id,
        metadata: { to },
      });
  ...
  async addReport(input: {
    ...
    return this.repo.withIsolation(async (tx) => {
      ...
      await this.audit.record(tx, {
        operation: "imaging.report.create",
  ...
  async verifyReport(reportId: string, _meta?: Record<string, unknown>) {
    ...
    if (!actor?.permissions?.includes("note:sign"))
      throw DomainError.forbidden("note:sign required to verify");
    return this.repo.withIsolation(async (tx) => {
      const r = await this.repo.verifyReport(reportId, tx);
      if (!r)
        throw DomainError.invalidState(
          "Report cannot be verified (already verified or missing)",
        );
      await this.audit.record(tx, {
        operation: "imaging.report.verify",
        resourceType: "imaging_report",
        resourceId: reportId,
        after: r,
      });
