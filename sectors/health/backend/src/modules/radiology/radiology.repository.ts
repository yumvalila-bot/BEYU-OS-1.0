...
export class RadiologyRepository extends BaseRepository {
  createOrder(input: Record<string, unknown>, tx: Tx) {
    ...
    return tx.query(
      ...
      [this.tenantContext.tenantId(), no, input.patient_id, input.encounter_id ?? null,
        input.provider_id ?? null, input.modality, input.body_part, input.laterality ?? null,
        input.clinical_indication ?? null, input.contrast ?? false,
  ...
  findOrder(id: string, tx?: Tx) {
    const q = (c: Tx) => c.query(`SELECT * FROM health.imaging_orders WHERE imaging_order_id=$1 AND ${this.notVoided("health.imaging_orders")}`, [id]).then((r: any[]) => r[0] ?? null);
    return tx ? q(tx) : this.withIsolation(q);
  }
  transitionOrder(id: string, to: string, tx: Tx) {
    const actor = this.actorId();
    const cid = this.correlationId();
    const stamp = to === "scheduled" ? "scheduled_at" : to === "final" ? "completed_at" : null;
    const set = stamp ? `status=$2, ${stamp}=now(), updated_by=$3, correlation_id=$4` : `status=$2, updated_by=$3, correlation_id=$4`;
    return tx.query(
      `UPDATE health.imaging_orders SET ${set} WHERE imaging_order_id=$1 AND ${this.notVoided("health.imaging_orders")} RETURNING *`,
      [id, to, actor, cid],
    ).then((r: any[]) => r[0] ?? null);
  }
  addReport(input: { imaging_order_id: string; findings: string; impression?: string }, tx: Tx) {
    const actor = this.actorId();
    const cid = this.correlationId();
    return tx.query(
      ...
      [this.tenantContext.tenantId(), input.imaging_order_id, input.findings, input.impression ?? null, actor, actor, cid],
    ).then((r: any[]) => r[0]);
  }
