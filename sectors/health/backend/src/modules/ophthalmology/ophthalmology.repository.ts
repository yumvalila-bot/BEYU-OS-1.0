...
export class OphthalmologyRepository extends BaseRepository {
  addExam(input: Record<string, unknown>, tx: Tx) {
    ...
    return tx.query(
      ...
      [this.tenantContext.tenantId(), input.patient_id, input.encounter_id ?? null,
        input.exam_date ?? null, input.provider_id ?? null,
        input.va_od ?? null, input.va_os ?? null, input.va_ou ?? null,
        input.va_pinhole_od ?? null, input.va_pinhole_os ?? null,
        input.refraction_od ?? null, input.refraction_os ?? null,
        input.iop_od ?? null, input.iop_os ?? null,
        input.slit_lamp_od ?? null, input.slit_lamp_os ?? null,
        input.fundus_od ?? null, input.fundus_os ?? null,
        input.diagnosis_od ?? null, input.diagnosis_os ?? null, input.diagnosis_ou ?? null,
        input.plan ?? null, input.follow_up_date ?? null, input.laterality_focus ?? null,
        input.notes ?? null, actor, cid],
    ).then((r: any[]) => r[0]);
  }
  ...
  find(id: string, tx?: Tx) {
    const q = (c: Tx) => c.query(`SELECT * FROM health.eye_exams WHERE exam_id=$1 AND ${this.notVoided("health.eye_exams")}`, [id]).then((r: any[]) => r[0] ?? null);
