...
describe("OphthalmologyService", () => {
  ...
  it("creates a structured eye exam preserving laterality (right/left/bilateral)", () =>
    bed.run(async () => {
      ...
      const e = (await svc.addExam({
        ...
        refraction_od: "-1.00", refraction_os: "-0.50",
        iop_od: 16, iop_os: 17,
        slit_lamp_od: "Clear", slit_lamp_os: "Clear",
        fundus_od: "Normal", fundus_os: "Normal",
        diagnosis_ou: "Myopia", plan: "Spectacles",
      })) as any;
  ...
  it("signing requires note:sign permission and prevents double-sign", () =>
    bed.run(async () => {
      const p = await bed.seedPatient();
      const e = (await svc.addExam({ patient_id: p.patient_id, laterality_focus: "right", diagnosis: "Cataract" })) as any;
