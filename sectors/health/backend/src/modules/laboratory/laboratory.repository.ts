...
export class LaboratoryRepository extends BaseRepository {
  listTests() {
    return this.withIsolation((tx) =>
      tx.query(`SELECT * FROM health.lab_tests WHERE ${this.notVoided("health.lab_tests")} ORDER BY name`),
    );
  }
  createTest(input: Record<string, unknown>, tx?: Tx) {
    ...
    const params = [this.tenantContext.tenantId(), input.code_system ?? "LOINC", input.code ?? null, input.name,
      input.specimen_type ?? null, input.unit ?? null, input.reference_text ?? null, actor, cid];
  ...
  listOrders(patientId?: string) {
    return this.withIsolation((tx) =>
      patientId
        ? tx.query(`SELECT * FROM health.lab_orders WHERE patient_id=$1 AND ${this.notVoided("health.lab_orders")} ORDER BY ordered_at DESC`, [patientId])
        : tx.query(`SELECT * FROM health.lab_orders WHERE ${this.notVoided("health.lab_orders")} ORDER BY ordered_at DESC LIMIT 200`),
    );
  }
  createOrder(input: { patient_id: string; encounter_id?: string; provider_id?: string; test_ids: string[]; clinical_info?: string; urgent?: boolean; idempotency_key?: string }, tx: Tx) {
    const actor = this.actorId();
    const cid = this.correlationId();
    ...
    return tx.query(
      ...
      [this.tenantContext.tenantId(), no, input.patient_id, input.encounter_id ?? null, input.provider_id ?? null,
        input.clinical_info ?? null, input.urgent ?? false, input.idempotency_key ?? null, actor, cid],
    ).then(async (r: any[]) => {
      const order = r[0];
  ...
  findByIdempotency(key: string) {
    ...
    return this.withIsolation((tx) =>
      tx.query(`SELECT * FROM health.lab_orders WHERE idempotency_key=$1 AND tenant_id=$2 AND ${this.notVoided("health.lab_orders")}`, [key, this.tenantContext.tenantId()]),
    ).then((r: any[]) => r[0] ?? null);
  }
  findOrder(id: string, tx?: Tx) {
    const q = (c: Tx) =>
      c.query(`SELECT * FROM health.lab_orders WHERE order_id=$1 AND ${this.notVoided("health.lab_orders")}`, [id]).then((r: any[]) => r[0] ?? null);
    return tx ? q(tx) : this.withIsolation(q);
  }
  transitionOrder(id: string, to: string, stampColumn?: string, tx?: Tx) {
    ...
    const q = (c: Tx) => c.query(sql, [id, to, actor, cid]).then((r: any[]) => r[0] ?? null);
    return tx ? q(tx) : this.withIsolation(q);
  }
  listOrderItems(orderId: string, tx?: Tx) {
    const q = (c: Tx) => c.query(`SELECT * FROM health.lab_order_items WHERE order_id=$1 AND ${this.notVoided("health.lab_order_items")}`, [orderId]);
    return tx ? q(tx) : this.withIsolation(q);
  }
  enterResult(itemId: string, result: { value_numeric?: number; value_text?: string; abnormal_flag?: string; comment?: string }, tx: Tx) {
    const actor = this.actorId();
    const cid = this.correlationId();
    return tx.query(
      `UPDATE health.lab_order_items
       ...
       RETURNING *`,
      [itemId, result.value_numeric ?? null, result.value_text ?? null, result.abnormal_flag ?? null, result.comment ?? null, actor, cid],
    ).then((r: any[]) => r[0] ?? null);
  }
