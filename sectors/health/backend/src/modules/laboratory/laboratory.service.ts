...
export class LaboratoryService {
  ...
  async createTest(input: Record<string, unknown>) {
    ...
    return this.repo.withIsolation(async (tx) => {
      ...
      await this.audit.record(tx, {
        operation: "lab.test.create",
        resourceType: "lab_test",
        resourceId: t.test_id as string,
        after: t,
      });
      return t;
    });
  ...
  listOrders(patientId?: string) {
    return this.repo.listOrders(patientId);
  }
  async createOrder(input: any) {
    if (!input.test_ids?.length)
      throw DomainError.validation("test_ids required");
    ...
    return this.repo.withIsolation(async (tx) => {
      ...
      await this.audit.record(tx, {
        operation: "lab.order.create",
        resourceType: "lab_order",
        resourceId: o.order_id,
        after: o,
      });
      return o;
    });
  ...
  async transition(id: string, to: string) {
    return this.repo.withIsolation(async (tx) => {
      ...
      if (!ORDER_TRANSITIONS[cur.status]?.has(to))
        throw DomainError.invalidState(
          `Cannot transition lab order from ${cur.status} to ${to}`,
        );
      const o = await this.repo.transitionOrder(id, to, STAMP[to], tx);
      await this.audit.record(tx, {
        operation: "lab.order.transition",
        resourceType: "lab_order",
        resourceId: id,
        metadata: { to },
      });
  ...
  async enterResult(itemId: string, result: any) {
    return this.repo.withIsolation(async (tx) => {
      ...
      if (!r)
        throw DomainError.invalidState(
          "Result cannot be entered (item already verified or missing)",
        );
      await this.audit.record(tx, {
        operation: "lab.result.enter",
  ...
  async verifyResult(itemId: string, _meta?: Record<string, unknown>) {
    return this.repo.withIsolation(async (tx) => {
      ...
      if (!r)
        throw DomainError.invalidState(
          "Result cannot be verified (no entered result or already verified)",
        );
      await this.audit.record(tx, {
        operation: "lab.result.verify",
        resourceType: "lab_result",
        resourceId: itemId,
        after: r,
      });
  ...
  async listOrderItems(orderId: string) {
    return this.repo.withIsolation((tx) =>
      this.repo.listOrderItems(orderId, tx),
    );
  }
}
