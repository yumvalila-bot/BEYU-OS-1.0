...
export type InvocationStatus =
  "recorded" | "submitted_to_hive" | "completed" | "failed" | "blocked";
...
export class AiGovernanceService {
  ...
  async invoke(input: InvocationInput): Promise<{
    ...
    const riskClass: RiskClass =
      input.riskClass ?? this.riskFor(input.taskType);
        ...
        const ins = await tx.query<{ invocation_id: string }>(
         ...
         RETURNING invocation_id`,
          [
            ...
            input.taskType,
            JSON.stringify({ fields: Object.keys(input.input ?? {}) }),
            inputHash,
            riskClass,
            cid,
          ],
  ...
  async recordHumanDecision(
    ...
    if (actor.userId === reviewerId) {
      throw DomainError.forbidden(
        "AI output cannot be approved by the invoking user (self-approval prohibited)",
      );
    }
    return withIsolation(
      this.db,
      ...
      async (tx) => {
        ...
        if (
          rows[0].human_approval_status === "approved" &&
          rows[0].risk_classification === "critical"
        ) {
          throw DomainError.requiresHumanDecision(
            "critical AI outputs already approved; overrides require a second reviewer (not yet implemented)",
          );
        }
        await tx.query(
  ...
  private riskFor(task: string): RiskClass {
    if (
      /(medication|prescrib|diagnos|differential|triage|sepsis|imaging_interpret)/i.test(
        task,
      )
    )
      return "high";
