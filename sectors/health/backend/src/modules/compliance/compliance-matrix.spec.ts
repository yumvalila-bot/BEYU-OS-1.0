/**
 * Compliance control matrix — machine-readable NABH-aligned and Tanzania-law
 * aligned engineering controls.  Honest classification: IMPLEMENTED means a
 * code-level control exists (validation, guard, adapter, audit record, gate);
 * PARTIALLY_IMPLEMENTED means the skeleton exists but coverage is incomplete;
 * MISSING means not yet built; EXTERNAL_BLOCKED requires external authority;
 * REQUIRES_HUMAN_APPROVAL requires human sign-off.
 *
 * The matrix is a registry — it does NOT claim NABH/ISO/legal accreditation.
 */
