...
describe("TelehealthService", () => {
  ...
  it("fail-closed: video_provider token/URL are NULL until integration is configured", () =>
    bed.run(async () => {
      ...
      const s = (await svc.createSession({
        patient_id: p.patient_id, scheduled_start: new Date(Date.now() + 3600_000).toISOString(),
        reason: "Follow-up", consent_obtained: true,
      })) as any;
  ...
  it("state machine requested->confirmed->in_progress->completed computes duration_sec", () =>
    bed.run(async () => {
      ...
      const s = (await svc.createSession({
        patient_id: p.patient_id, scheduled_start: new Date(Date.now() + 3600_000).toISOString(),
        reason: "Acute", consent_obtained: true,
      })) as any;
      ...
      await bed.conn.query(`UPDATE health.telehealth_sessions SET started_at=now() - interval '10 minutes' WHERE session_id=$1`, [s.session_id]);
  ...
  it("rejects invalid state transition (e.g., completed before in_progress)", () =>
    bed.run(async () => {
      const p = await bed.seedPatient();
      const s = (await svc.createSession({ patient_id: p.patient_id, scheduled_start: new Date(Date.now() + 3600_000).toISOString(), reason: "x", consent_obtained: true })) as any;
      await expect(svc.transition(s.session_id, "completed")).rejects.toBeInstanceOf(DomainError);
    }));

  it("blocks session creation when consent_obtained is false", () =>
    bed.run(async () => {
      const p = await bed.seedPatient();
      await expect(svc.createSession({ patient_id: p.patient_id, scheduled_start: new Date().toISOString(), reason: "x", consent_obtained: false })).rejects.toBeInstanceOf(DomainError);
    }));
});
