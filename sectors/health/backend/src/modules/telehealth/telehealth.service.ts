...
export class TelehealthService {
  ...
  async createSession(input: any) {
    ...
    if (!input.consent_obtained)
      throw DomainError.validation(
        "Telehealth consent is required before a session can be created",
      );
    ...
    return atomicWrite(
      ...
      "telehealth_session",
      (tx) => this.repo.createSession(input, tx),
      (r) => r.session_id,
    );
  }
  async transition(
    ...
    return atomicTransition(
      ...
      async (sid, tto, tx) => {
        ...
        if (tto === "completed") {
          ...
          p.ended_at = new Date();
          p.duration_sec = cur?.started_at
            ? Math.max(
                0,
                Math.floor(
                  (Date.now() - new Date(cur.started_at).getTime()) / 1000,
                ),
              )
            : 0;
        }
