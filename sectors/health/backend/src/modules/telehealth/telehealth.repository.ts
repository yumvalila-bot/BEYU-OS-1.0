...
export class TelehealthRepository extends BaseRepository {
  ...
  findSession(id: string, tx?: Tx) {
    const q = (c: Tx) =>
      c
        .query(
          `SELECT * FROM health.telehealth_sessions WHERE session_id=$1 AND ${this.notVoided("health.telehealth_sessions")}`,
          [id],
        )
        .then((r: any[]) => r[0] ?? null);
  ...
  updateSession(id: string, patch: Record<string, unknown>, tx: Tx) {
    ...
    return tx
      .query(
        `UPDATE health.telehealth_sessions SET ${sets.join(",")} WHERE session_id=$1 AND ${this.notVoided("health.telehealth_sessions")} RETURNING *`,
        params,
      )
      .then((r: any[]) => r[0] ?? null);
  }
