...
export class AmbulanceRepository extends BaseRepository {
  ...
  listVehicles() {
    return this.withIsolation((tx) =>
      tx.query(
        `SELECT * FROM health.vehicles WHERE ${this.notVoided("health.vehicles")} ORDER BY plate`,
      ),
    );
  }
  ...
  findRequest(id: string, tx?: Tx) {
    const q = (c: Tx) =>
      c
        .query(
          `SELECT * FROM health.ambulance_requests WHERE request_id=$1 AND ${this.notVoided("health.ambulance_requests")}`,
          [id],
        )
        .then((r: any[]) => r[0] ?? null);
  ...
  updateRequest(id: string, patch: Record<string, unknown>, tx: Tx) {
    ...
    return tx
      .query(
        `UPDATE health.ambulance_requests SET ${sets.join(",")} WHERE request_id=$1 AND ${this.notVoided("health.ambulance_requests")} RETURNING *`,
        params,
      )
      .then((r: any[]) => r[0] ?? null);
  }
