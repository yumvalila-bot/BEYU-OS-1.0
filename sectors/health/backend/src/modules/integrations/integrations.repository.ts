...
export class IntegrationsRepository extends BaseRepository {
  ...
  listStatuses() {
    return this.withIsolation((tx) => tx.query(`SELECT * FROM health.integration_status WHERE tenant_id=$1`, [this.tenantContext.tenantId()]));
  }
  setStatus(provider: string, state: string, lastError?: string | null, tx?: any) {
    ...
    const sql = `INSERT INTO health.integration_status (tenant_id, provider, state, last_error, last_checked_at)
                 ...
                 RETURNING *`;
    const params = [this.tenantContext.tenantId(), provider, state, lastError ?? null];
