...
export class PharmacyRepository extends BaseRepository {
  listItems() {
    return this.withIsolation((tx) =>
      tx.query(`SELECT * FROM health.pharmacy_items WHERE ${this.notVoided("health.pharmacy_items")} ORDER BY name`),
    );
  }
  createItem(input: Record<string, unknown>, tx?: Tx) {
    ...
    const params = [this.tenantContext.tenantId(), input.sku, input.name, input.generic_name ?? null,
      input.form ?? null, input.strength ?? null, input.unit ?? "each",
      input.controlled ?? false, input.requires_rx ?? true,
      input.code ?? null, input.code_system ?? "RXNORM", actor, cid];
  ...
  findItemBySku(sku: string, tx?: Tx) {
    ...
    const q = (c: Tx) => c.query(sql, [sku, this.tenantContext.tenantId()]).then((r: any[]) => r[0] ?? null);
    return tx ? q(tx) : this.withIsolation(q);
  }
  ...
  receiveStock(input: Record<string, unknown>, tx: Tx) {
    ...
    return tx.query(
      ...
      [this.tenantContext.tenantId(), input.item_id, input.lot_number, input.expiry_date, input.qty, actor, cid],
    ).then((r: any[]) => r[0]);
  }
  recordMovement(mv: Record<string, unknown>, tx: Tx) {
    ...
    return tx.query(
      ...
      [this.tenantContext.tenantId(), mv.item_id, mv.batch_id ?? null, mv.movement_type, mv.qty,
       mv.reference_type ?? null, mv.reference_id ?? null, mv.note ?? null,
       mv.idempotency_key ?? null, actor, cid],
    ).then((r: any[]) => r[0]);
  }
  getStockLevel(item_id: string, tx: Tx) {
    return tx.query<{ on_hand: number }>(
  ...
  createDispense(input: Record<string, unknown>, tx: Tx) {
    ...
    return tx.query(
      ...
      [this.tenantContext.tenantId(), input.encounter_id ?? null, input.medication_id,
        input.patient_id, input.item_id, input.qty, input.dose_given ?? null, actor,
  ...
  findDispenseByIdempotency(key: string) {
    ...
    return this.withIsolation((tx) =>
      tx.query(`SELECT * FROM health.dispenses WHERE idempotency_key=$1 AND tenant_id=$2 AND ${this.notVoided("health.dispenses")}`,
  ...
  findMedicationById(id: string, tx: Tx) {
    return tx.query(`SELECT * FROM health.medications WHERE medication_id=$1 AND ${this.notVoided("health.medications")}`, [id])
      .then((r: any[]) => r[0] ?? null);
  }
}
