...
describe("Database migrations (Phase 2B)", () => {
  it("clean DB: all up migrations apply without error", async () => {
    ...
    const r = await db.query<{ n: number }>(`SELECT count(*)::int AS n FROM information_schema.tables WHERE table_schema='health'`);
  ...
  it("up migrations are idempotent when re-applied (safe to re-run bootstrap)", async () => {
    const db = new PGlite();
    for (const f of listMigs("up")) await db.exec(fs.readFileSync(path.join(MIG_DIR, f), "utf8"));
  ...
  it("down migrations execute without error to a clean state (disposable environments only)", async () => {
    const db = new PGlite();
    for (const f of listMigs("up")) await db.exec(fs.readFileSync(path.join(MIG_DIR, f), "utf8"));
    const before = await db.query<{ n: number }>(`SELECT count(*)::int AS n FROM information_schema.tables WHERE table_schema='health'`);
    ...
    const healthTables = await db.query<{ n: number }>(`SELECT count(*)::int AS n FROM information_schema.tables WHERE table_schema='health'`);
