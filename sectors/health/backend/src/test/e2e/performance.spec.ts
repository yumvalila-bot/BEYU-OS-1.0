...
describe("Performance measurements (local PGlite, in-process)", () => {
  ...
  afterAll(async () => {
    ...
    for (const [op, s] of Object.entries(samples)) {
      metrics[op] = {
        ...
        p50_ms: Math.round(percentile(s.samples, 0.5)),
      ...
      JSON.stringify(
        {
          ...
          disclaimer:
            "Local in-process PGlite measurements; NOT production-scale.",
          metrics,
        },
        null,
        2,
  ...
  it("health/live probe", async () => {
    await measure(
      "health.live",
      () => request(h.app.getHttpServer()).get("/health/live"),
      20,
    );
  });
  it("auth/login (post-warmup)", async () => {
    await measure(
      ...
      async () => {
        ...
        return request(h.app.getHttpServer()).post("/auth/login").send({
          email,
          password: "CorrectHorseBattery1!",
          tenantCode: "test",
        });
    ...
    await measure(
      ...
      () =>
        request(h.app.getHttpServer())
          ...
          .set("Authorization", `Bearer ${token}`)
          .send({
            medical_record: `MRN-PERF-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
            given_name: "Perf",
            family_name: "Test",
          }),
      5,
  ...
  it("audit-write (via rate-limit event on blocked request)", async () => {
    await measure(
      "audit.read_probe",
      () => request(h.app.getHttpServer()).get("/health/ready"),
      10,
    );
  });
});
