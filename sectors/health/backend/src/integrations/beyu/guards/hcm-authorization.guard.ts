...
export const RequireHcmPractitioner = (action: string, opts: { scope?: string[] } = {}) => {
  ...
  return (target: any, key?: any, desc?: any) => {
    if (desc) {
      ...
      if (opts.scope) Reflect.defineMetadata(HCM_SCOPE_KEY, opts.scope, desc.value);
      return desc;
    }
...
export class HcmAuthorizationGuard implements CanActivate {
  constructor(private readonly hcm: HcmAdapter, private readonly reflector: Reflector) {}

  async canActivate(ctx: ExecutionContext): Promise<boolean> {
    const action = this.reflector.getAllAndOverride<string>(HCM_ACTION_KEY, [ctx.getHandler(), ctx.getClass()]);
    if (!action) return true; // no HCM requirement
    const scope = this.reflector.getAllAndOverride<string[]>(HCM_SCOPE_KEY, [ctx.getHandler(), ctx.getClass()]) ?? [];
