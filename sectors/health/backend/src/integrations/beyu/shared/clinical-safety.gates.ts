...
export class ClinicalSafetyGates {
  ...
  async pharmacyDispense(
    ...
    const scope = [
      "rx:dispense",
      ...(input.controlledSubstance ? ["rx:controlled"] : []),
    ];
    ...
    if (
      input.controlledSubstance &&
      input.requiresDualControl &&
      !input.secondReviewerGlobalUserId
    ) {
      return deny(
        "DUAL_CONTROL",
        "DUAL_CONTROL_REQUIRED_FOR_CONTROLLED_SUBSTANCE",
      );
    }
  ...
  async labRelease(
    ...
    if (!hcmRes.authorized) return deny("HCM", hcmRes.reason ?? "HCM_DENIED");
    if (!input.specimenIntegrity)
      return deny("SPECIMEN", "SPECIMEN_INTEGRITY_FAILED");
    if (!input.analyzerAuthorized)
      return deny("ANALYZER", "ANALYZER_NOT_AUTHORIZED");
    if (!input.qcPassed) return deny("QC", "QC_NOT_PASSED");
    if (!input.verifiedByGlobalUserId)
      return deny("VERIFICATION", "LAB_RESULT_VERIFICATION_REQUIRED");
  ...
  async radiologyVerify(
    ...
    if (!hcmRes.authorized) return deny("HCM", hcmRes.reason ?? "HCM_DENIED");
    if (!input.equipmentAuthorized)
      return deny("EQUIPMENT", "EQUIPMENT_NOT_AUTHORIZED");
    if (!input.radiationSafetyCleared)
      return deny("RADIATION", "RADIATION_SAFETY_NOT_CLEARED");
    if (!input.dicomIdentityLinked)
      return deny("DICOM", "DICOM_PATIENT_IDENTITY_NOT_LINKED");
    if (!input.doseCaptured) return deny("DOSE", "RADIATION_DOSE_NOT_CAPTURED");
    if (!input.verifiedByGlobalUserId)
      return deny("VERIFICATION", "RADIOLOGY_REPORT_VERIFICATION_REQUIRED");
    if (input.criticalFinding && !input.metadata?.criticalEscalationLogged) {
      return deny(
        "CRITICAL_ESCALATION",
        "CRITICAL_FINDING_ESCALATION_REQUIRED",
      );
    }
    return allow();
  }

  async ophthalmologyDispense(
    input: SafetyGateInput & {
    ...
    const hcmRes = await this.hcm.authorizeClinicalActor({
      action: input.action,
      facilityId: input.facilityId,
      requiredScope: ["optical:dispense"],
    });
    if (!hcmRes.authorized) return deny("HCM", hcmRes.reason ?? "HCM_DENIED");
    if (!input.prescriptionValid)
      return deny("RX", "OPTICAL_PRESCRIPTION_INVALID");
    if (!input.practitionerScopeOk)
      return deny("SCOPE", "PRACTITIONER_SCOPE_INSUFFICIENT");
    if (!input.deviceLinked) return deny("DEVICE", "OPTICAL_DEVICE_NOT_LINKED");
    if (!input.deviceTraceability)
      return deny("TRACEABILITY", "MEDICAL_DEVICE_TRACEABILITY_REQUIRED");
    if (!input.dispensingVerified)
      return deny("DISPENSE", "OPTICAL_DISPENSING_VERIFICATION_REQUIRED");
    return allow();
  }

  async dialysisTreatment(
    input: SafetyGateInput & {
    ...
    const hcmRes = await this.hcm.authorizeClinicalActor({
      action: input.action,
      facilityId: input.facilityId,
      requiredScope: ["dialysis:treat"],
    });
    if (!hcmRes.authorized) return deny("HCM", hcmRes.reason ?? "HCM_DENIED");
    if (!input.machineAuthorized)
      return deny("MACHINE", "DIALYSIS_MACHINE_NOT_AUTHORIZED");
    if (!input.maintenanceCurrent)
      return deny("MAINTENANCE", "DIALYSIS_MACHINE_MAINTENANCE_OVERDUE");
    if (!input.waterQualityPassed)
      return deny("WATER", "DIALYSIS_WATER_QUALITY_FAILED");
    if (!input.patientMatched)
      return deny("PATIENT", "PATIENT_IDENTITY_MISMATCH");
    if (!input.treatmentParamsValid)
      return deny("PARAMS", "INVALID_TREATMENT_PARAMETERS");
    if (input.adverseEventOpen)
      return deny("ADVERSE", "OPEN_ADVERSE_EVENT_MUST_BE_RESOLVED");
  ...
  async generalClinicalWrite(
    ...
    if (!hcmRes.authorized) return deny("HCM", hcmRes.reason ?? "HCM_DENIED");
    if (input.patientIdentityConfirmed === false)
      return deny("PATIENT", "PATIENT_IDENTITY_UNCONFIRMED");
    return allow();
  }
