...
it.skipIf(!adminUrl)("0058 preserves predecessor governance history and introduces no lifecycle transitions",async()=>{
 const root=process.cwd(),name=`beyu_body_lifecycle_upgrade_${randomUUID().replaceAll("-","")}`;
 ...
 mkdirSync(join(root,"tmp/governance"),{recursive:true});const dir=mkdtempSync(join(root,"tmp/governance/body-lifecycle-upgrade-"));mkdirSync(join(dir,"drizzle"));
...
// 0066 belongs in the predecessor baseline. The migration under test (0058)
// remains the ONLY upgrade step applied after the predecessor state.
for(const f of readdirSync(join(root,"drizzle")).filter(f=>/^\d+.*\.sql$/.test(f)&&(Number(f.slice(0,4))<=57||f==="0066_shared_search_fulltext.sql")))copyFileSync(join(root,"drizzle",f),join(dir,"drizzle",f));
 ...
 try{
  ...
  copyFileSync(join(root,"drizzle/0058_governance_body_lifecycle.sql"),join(dir,"drizzle/0058_governance_body_lifecycle.sql"));
  ...
  // 60 = 0000-0057 (58) + 0066 shared-search baseline (1) + 0058 under test (1).
