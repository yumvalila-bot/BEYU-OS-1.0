...
it.skipIf(!adminUrl)("0056 upgrades real consented initial appointments without rewriting any historical authority",async()=>{
 ...
 // 0066 belongs in the predecessor baseline. The migration under test (0056)
 // remains the ONLY upgrade step applied after the predecessor state.
 for(const f of readdirSync(join(root,"drizzle")).filter(f=>/^\d+.*\.sql$/.test(f)&&(Number(f.slice(0,4))<=55||f==="0066_shared_search_fulltext.sql")))copyFileSync(join(root,"drizzle",f),join(dir,"drizzle",f));
 ...
 try{
  ...
  // 58 = 0000-0055 (56) + 0066 shared-search baseline (1) + 0056 under test (1).
