...
it.skipIf(!adminUrl)("0054 preserves actual appointment history and blocks unknown provenance without guessing identities", async () => {
 const root = process.cwd(), name = `beyu_appointment_origin_upgrade_${randomUUID().replaceAll("-", "")}`;
 ...
 try {
  ...
  for (const id of ["GAP_LEGACY_NOMINATED", "GAP_LEGACY_APPROVED"]) {
   await client.query(`insert into governance_appointments(id,body_id,nominee_user_id,party_id,seat_role,voting_rights,appointed_on,retired_on,document_id,document_version,document_checksum,classification,rationale,nominated_by_user_id)
    select $1,$2,$3,$4,'MEMBER',true,CURRENT_DATE,'2030-12-31',id,version,checksum,classification,'Preserve exact historical appointment terms',$5 from documents where id='DOC_D4'`, [id,body.id,nominee.id,nominee.party_id,actor]);
  ...
  await client.query("update governance_appointments set status='APPROVED',revision=2,approved_by_user_id=$1,resolution_id='RES_GAP_LEGACY_APPROVED' where id='GAP_LEGACY_APPROVED'",[approver]);
  const before = (await client.query("select * from governance_appointments order by id")).rows;
  copyFileSync(join(root,"drizzle/0054_governance_appointment_origin.sql"),join(dir,"drizzle/0054_governance_appointment_origin.sql"));
  ...
  await runtimeScope(approver, async () => {
   ...
   await expect(client.query("update governance_appointments set status='APPROVED',revision=2,approved_by_user_id=$1,approved_by_party_id=$2,resolution_id='RES_GAP_LEGACY_NOMINATED' where id='GAP_LEGACY_NOMINATED'",[approver,approvingParty])).rejects.toMatchObject({ code: "23514", message: expect.stringContaining("Original nominating party is unknown") });
  });
  await runtimeScope(nominee.id, async () => {
   await expect(client.query("update governance_appointments set status='ACCEPTED',revision=3,accepted_at=now() where id='GAP_LEGACY_APPROVED'")).rejects.toMatchObject({ code: "23514", message: expect.stringContaining("Original nominating party is unknown") });
  });
  await runtimeScope(nominee.id, async () => {
