...
async function main() {
  ...
  await adminDb
    ...
    .values([
      ...
      { id: fixedId(ID_PREFIX.osRegistry, "SEARCH"), code: "SHARED_SEARCH", name: "Shared Search", kind: "SHARED_CAPABILITY", purpose: "One governed cross-OS full-text search over records the principal can already read. A shared BEYU OS capability — never a Search OS: it owns no data, adds no index of its own beyond the FTS columns on existing tables, grants no new capability, and every result is a live governed read through /api/v1/search.", ownerRole: "PLATFORM_ADMIN", authorityScope: "ENTERPRISE_WIDE", dataAuthority: [], dependencies: ["BEYU_OS"], apis: ["/api/v1/search"], events: [], complianceFrameworks: [] },
