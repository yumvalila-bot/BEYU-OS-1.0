...
async function main() {
  ...
  await adminDb
    ...
    .values([
      ...
      { id: E.ujenzi, tenantId: T.ujenzi, code: "BEYU-UJZ", legalName: "BEYU Ujenzi Ltd", entityType: "OPERATING_COMPANY", parentEntityId: E.tzHold, countryCode: "TZ", jurisdictionId: fixedId(ID_PREFIX.jurisdiction, "TZ-NAT"), registrationNumber: "TZ-161220885", incorporationDate: "2021-03-01", functionalCurrency: "TZS", sectorCode: "CONSTRUCTION", effectiveFrom: "2021-03-01" },
  ...
  await adminDb
    ...
    .values([
      ...
      { id: fixedId(ID_PREFIX.ownership, "TZH_UJENZI"), tenantId: T.tz, ownedEntityId: E.ujenzi, ownerEntityId: E.tzHold, ownershipType: "DIRECT", economicPct: "100", votingPct: "100", effectiveFrom: "2021-03-01", provenance: "Share certificate U-001", recordedBy: "SEED/CONSTITUTIONAL_BOOTSTRAP" },
  ...
  const people: Person[] = [
    ...
    { key: "HASSAN_KIMARO", name: "Hassan Kimaro", given: "Hassan", family: "Kimaro", email: "ujenzi.ops@beyu.os", role: "SECTOR_OPERATOR", tenant: T.ujenzi },
  ...
  await adminDb
    ...
    .values([
      ...
      { id: fixedId(ID_PREFIX.osRegistry, "UJENZI_OS"), code: "UJENZI_OS", name: "BEYU Ujenzi OS", kind: "SECTOR_OS", purpose: "One construction Sector OS: projects, design, engineering, BIM, BOQ, procurement, site, HSE, QA/QC, progress, interior, FF&E, government tracking and Vision 2050 alignment. Not inner operating systems. Finance OS remains the journal.", ownerRole: "SECTOR_OPERATOR", authorityScope: "CONSTRUCTION_OPERATIONS", dataAuthority: ["CONSTRUCTION_PROJECT", "LAND_SITE", "SOIL_TEST", "ENGINEERING_CALCULATION", "BOQ", "SITE_PROGRESS"], dependencies: ["BEYU_OS", "FINANCE_OS", "SHARED_HCM"], apis: ["/api/v1/ujenzi/*"], events: ["PROJECT_CREATED"], complianceFrameworks: ["TZ_BUILDING"], lifecycle: "ACTIVE" },
  ...
  await adminDb
    ...
    .values(
      [
        ...
        ["Construction operations", "UJENZI_OS", "ujenzi.projects", ["BEYU_OS"]],
  ...
  await adminDb
    ...
    .values([
      ...
      { id: fixedId(ID_PREFIX.adr, "ADR5"), adrNumber: 5, title: "Life insurance is governed inside the Family Office as Protection & Insurance — never an Insurance OS", status: "ACCEPTED", context: "Family-office life-insurance protection, succession liquidity and claims needed a first-class record set. A standalone Insurance OS was considered and refused: it would duplicate identity, governance, documents, audit and Finance, and fragment the family wealth picture the Family Office exists to consolidate.", decision: "A governed Protection & Insurance capability inside BEYU OS → Family Office (schema family-office-protection.ts, engine family/office/protection-insurance, API /api/v1/family-office/protection/*): policies with the five ownership roles recorded separately, insurance beneficiary designations DISTINCT from the trust register, premium obligations, reviews, a claims ledger with a monotonic proceeds machine, and deterministic explainable protection-gap assessments. Death benefits are contingent protection and can never be summed into wealth or liquidity; only recorded-received proceeds enter liquidity. Finance OS stays the sole accounting authority (CAP_POSTING not referenced by the domain); HCM stays canonical for employee identity; Risk stays canonical for risk; documents, events and audit are consumed, never duplicated.", consequences: "One more additive Family Office domain on the shared kernel; cross-currency aggregates are refused; legal/tax outcomes are recorded as cited professional review, never derived.", alternatives: "Insurance OS (rejected: OS proliferation and duplicate masters); modeling insurance inside Finance (rejected: contingent protection is not accounting); reusing the trust beneficiary register for designations (rejected: distinct legal relationships must not share entitlement).", securityAnalysis: "RBAC/ABAC on five new permissions with beneficiary.manage at MFA step-up; RLS tenant isolation on all nine tables via the canonical policy with a self-verifying migration; no AI tool path to any protection write permission.", complianceAnalysis: "No jurisdiction-specific legal or tax conclusions encoded; professional advice is recorded as cited evidence; audit and event trails cover every sensitive change.", rollbackPlan: "Additive-only: remove the domain's routes/UI and retire the nine tables through a governed deprecation migration; no existing table, route, permission or data was altered.", decidedBy: "Architecture Review Board", decidedOn: "2026-09-11" },
    ])
    .onConflictDoNothing();
