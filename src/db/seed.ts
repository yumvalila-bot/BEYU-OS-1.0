...
async function main() {
  ...
  // Bootstrap memory: every record carries its content checksum and the
  // attributed bootstrap writer (Iteration 11 memory governance).
  const bootstrapWriter = fixedId(ID_PREFIX.user, "GRACE_KILELE");
  const knowledgeSeed = [
  ...
  await db
    ...
    .values(knowledgeSeed.map((row) => ({
      ...row,
      contentChecksum: sha256(row.content),
      createdByUserId: bootstrapWriter,
      updatedByUserId: bootstrapWriter,
    })))
    .onConflictDoNothing();
