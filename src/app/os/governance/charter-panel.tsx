...
export function CharterPanel({ bodyId, canManage, userId, charters, quorum, majority, composition, initial = false }: {
  initial?: boolean; bodyId: string; canManage: boolean; userId: string; charters: Charter[]; quorum: number; majority: string; composition: string;
}) {
  ...
  return <details className="mt-4 border-t border-slate-500/20 pt-3" data-charter-body={bodyId}>
    ...
    <p className="my-2 text-xs beyu-muted">{composition}. {initial ? "The recorded superior must approve the initial charter. APPROVED is not effective and cannot activate this body." : "Adoption records a voted charter;"} it cannot grant a seat or change canonical voting rules.</p>
    ...
    {charters.map((c) => <article key={c.id} data-charter-id={c.id} className="my-3 space-y-2 rounded border border-slate-500/30 p-2 text-xs">
      ...
      {canManage && c.terms && (c.status === "DRAFT" || (c.status === "IN_REVIEW" && c.createdByUserId !== userId)) && <form className="space-y-2" onSubmit={(e) => {
        ...
        <button disabled={busy} className="rounded border border-[#b08d1c] p-2 disabled:opacity-40">{c.status === "DRAFT" ? "Submit charter for review" : initial ? "Record superior charter approval" : "Record independent charter adoption"}</button>
      </form>}
    </article>)}
